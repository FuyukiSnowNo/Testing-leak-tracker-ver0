...
def scrape(self, since: Optional[datetime] = None) -> list[LeakItem]:
   ...
   items = []
   for acct in accounts:
       username = acct["username"] if isinstance(acct, dict) else str(acct)
       items += self._scrape_account(username, since)
       time.sleep(getattr(config, "INSTAGRAM_DELAY", 5))
   logger.info(f"Instagram total: {len(items)} posts")
   return items
...
class InstagramScraper:
    ...
    def __init__(self):
        ...
        if session_file:
            try:
                ...
                logger.warning("Instagram session load failed: " + str(e))
    ...
    def scrape(self, since=None):
        ...
        for acct in accounts:
            ...
            items += self._scrape_account(username, since)
            time.sleep(getattr(config, "INSTAGRAM_DELAY", 5))
        logger.info("Instagram total: " + str(len(items)) + " posts")
        return items
 
    def _scrape_account(self, username, since):
        items = []
        try:
            profile = self._il.Profile.from_username(self.L.context, username)
            count = 0
            for post in profile.get_posts():
                ...
                item = LeakItem(
                    title=first_line or ("@" + username + " post"),
                    url="https://www.instagram.com/p/" + post.shortcode + "/",
                    source=self.source_id,
                    source_name="IG @" + username,
                    summary=summary,
                    published_at=pub,
                    image_url=img_url,
                )
            ...
            logger.error("Instagram @" + username + ": " + str(e))
